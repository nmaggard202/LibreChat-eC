...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    reader.onload = (e) => {
      ...
      if (validator) {
        setStatus('success');
        setStatusColor('text-red-500 dark:text-red-500');
      }
  ...
  return (
    <>
      <button
        ...
        onClick={handleClick}
        className={cn(
          'mr-1 flex h-auto cursor-pointer items-center rounded bg-transparent px-2 py-1 text-xs font-normal transition-colors hover:bg-gray-100 hover:text-red-600 focus:ring-ring dark:bg-transparent dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-red-500',
          statusColor,
          containerClassName,
        )}
